      ...
      try {
        ...
        if (activeFamily) {
          ...
          if (activitiesData.status === "fulfilled") {
            const val = activitiesData.value?.data || activitiesData.value;
            setActivities(Array.isArray(val) ? val : val?.results ?? []);
          }
          if (eventsData.status === "fulfilled") {
            const val = eventsData.value?.data || eventsData.value;
            setEvents(Array.isArray(val) ? val : val?.results ?? []);
          }
          if (birthdaysData.status === "fulfilled") {
            const val = birthdaysData.value?.data || birthdaysData.value;
            setBirthdays(Array.isArray(val) ? val : val?.results ?? []);
            setTodayBirthdays(val?.today ?? []);
          }
        }
      } catch (err) {
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-stone-200/80 pb-5">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-1.5 bg-brand-50 hover:bg-brand-100 text-brand-700 text-xs font-semibold px-3 py-2 rounded-xl transition-colors active:scale-95 cursor-pointer"
            >
              ...
              <div className="flex items-center gap-5 text-center sm:text-left">
                ...
                <div>
                  ...
                  <h2 className="text-xl sm:text-2xl font-black tracking-tight drop-shadow-sm text-white">
                    Happy Birthday, {todayBirthdays.map((b) => b.member || b.name).join(" & ")}!
                  </h2>
                        ...
                        >
                          <span
                            className={`w-2 h-2 rounded-full shrink-0 ${
                              family.is_active !== false ? "bg-emerald-500 animate-pulse" : "bg-amber-500"
                            }`}
                          />
                          {family.is_active !== false ? "Active Workspace" : "Inactive Workspace"}
                        </span>
                      </div>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="text-xl sm:text-2xl font-bold text-stone-900 mt-0.5">
                        {stats?.posts ?? stats?.total_posts ?? 0}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-stone-50/80 border border-stone-200/60 rounded-2xl p-4 flex items-center gap-3.5 hover:border-amber-400 hover:bg-white hover:shadow-md transition-all duration-200 group">
                    ...
                    <div>
                      ...
                      <div className="text-base sm:text-lg font-bold text-stone-900 mt-0.5 flex items-center gap-2">
                        ...
                        />
                        {family.is_active !== false ? "Active" : "Inactive"}
                      </div>
                    </div>
                  </div>
                </div>
                      ...
                      >
                        <div className="flex items-center gap-2.5 min-w-0">
                          ...
                          <p className="text-xs text-stone-800 truncate">
                            <span className="font-semibold text-stone-900">{act.actor || act.user || "Member"}</span>{" "}
                            <span className="text-stone-600">{act.action || act.description || "performed an action"}</span>
                          </p>
                        </div>

                        <span className="text-[10px] text-stone-400 font-medium shrink-0 flex items-center gap-1">
                          <Clock className="w-3 h-3 text-stone-400" />
                          {formatShortTime(act.timestamp || act.created_at)}
                        </span>
                      </div>
                    ))}
                  </div>
                      ...
                      {events.map((evt, index) => (
                        ...
                        >
                          <div className="flex items-center gap-2.5">
                            ...
                            <div>
                              <p className="font-bold text-stone-900 text-sm">{evt.title || evt.name}</p>
                              <p className="text-xs text-amber-700 font-semibold mt-0.5">
                                {evt.date || evt.start_date}
                              </p>
                            </div>
                          </div>
                        </div>
                      ...
                      {birthdays.map((bday, index) => (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="font-bold text-stone-900 text-sm">{bday.member || bday.name || bday.user}</p>
                              <p className="text-xs text-rose-700 font-semibold mt-0.5">
                                {bday.birthday || bday.date}
                              </p>
                            </div>
                          </div>
                        </div>
